...
- `lib/feeds.ts` — regex-parses the public YouTube and SoundCloud feeds (no API keys). Sends a browser User-Agent because YouTube 429s datacenter IPs otherwise. **YouTube retired its public RSS feeds** (`/feeds/videos.xml` now 404s for every channel), so `getLatestYouTubeVideos()` tries RSS first — it self-heals if YouTube brings it back — and otherwise falls back to scraping the `ytInitialData` JSON out of `youtube.com/@swarthy_dj/videos`. That path has no exact publish date, only YouTube's relative age ("hace 9 días"), which travels as `VideoItem.publishedLabel` and is what `Mixes` renders when `publishedAt` is empty. Returns `[]` on any failure; `Mixes` falls back to plain channel links. `Mixes` renders the newest item of each platform plus a grid of up to 6 more, interleaved so one platform never dominates. Every player is a **facade**: the thumbnail is a button and the `<iframe>` is only mounted on click, so YouTube/SoundCloud third-party cookies are never set on a plain visit. YouTube serves thumbnails from `i.ytimg.com` *and* `i1/i2/i3/i4.ytimg.com`, hence the `*.ytimg.com` `remotePattern` in `next.config.ts`.
...
- Keeper has no venue photo (`Venue.img` is optional and it falls back to a typographic card). The previous image was a scanned VIP consumption card: it clashed with the rest of the grid **and carried a third party's printed name**, so it was deleted from `public/` rather than left unreferenced. Any replacement must be an actual venue photo with no personal data in frame.
