...
- `app/page.tsx` — async server component that fetches the YouTube + SoundCloud feeds (`lib/feeds.ts`, revalidated hourly) and composes the sections in order: Hero → Stats → About → Dates → Mixes → Formats → Venues → Testimonials → Socials → Faq → Contact → FinalCta. Reorder by editing this file alone; sections do not depend on each other (FinalCta embeds BookingForm). `Dates` and `Testimonials` render nothing while `site.dates` / `site.testimonials` are empty, so the page degrades to its previous shape.
